...
class UserFormService extends Service {
  ...
  async list(fields) {
    ...
    let data = []
    if(!related){
      const users = await this._userRespository.findAll()
      const usersForms = await this._userFormRepository.list(formid)
      for(const user of users){
        const userRelated = usersForms.find(userForm => userForm.userid === user.id)
    ...
    const usersFormsList = await this._userFormRepository.list(formid)
