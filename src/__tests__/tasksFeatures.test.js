...
describe("TASKS FEATURES Tests", () => {
  ...
  beforeAll(async () => {
    try{
      ...
      leadStatus = await LeadStatus.create(mocks.mockLeadStatus()[0])
  ...
  describe("UPDATE TASK Tests", () => {
    ...
    test(`Should return 400 if invalid userid has been provided`, async () => {
      ...
      const body = mocks.mockTask("Invalid Userid", lead.id, taskType.id)
      body.id = task.id
      body.resolutionDate = new Date().toISOString()
      const req = mocks.mockReq(body, null, null, locals)
      const res = mocks.mockRes()
    ...
    test(`Should return 400 if invalid userid has been provided`, async () => {
      ...
      const body = mocks.mockTask(adminUser.id, "invalid leadid", taskType.id)
      body.id = task.id
      body.resolutionDate = new Date().toISOString()
      const req = mocks.mockReq(body, null, null, locals)
      const res = mocks.mockRes()
